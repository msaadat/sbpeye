...
import { adoptMyProviderForDeployment, getProviderModels, getSettings, saveSettings, testEmbeddingConnection, testSettingsConnection } from '@/lib/api'
...
const { refreshLlmDebugState, setLlmDebugState } = useLlmDebugState()
...
async function adoptMine(): Promise<void> {
  adopting.value = true
  try {
    await adoptMyProviderForDeployment()
    await loadSettings()
    toast.add({
      severity: 'success',
      summary: 'Copied from your settings',
      detail: 'Corpus generation now uses the same provider and key as your chat.',
      life: 5000,
    })
  } catch (error) {
    toast.add({
...
<template>
  <section class="deployment-ai">
    <div class="deployment-header">
      <p class="heading-note">
        The credentials admin-triggered work runs on — summaries, tags, checklists and law
        generation, all of which write the shared corpus. Nobody's chat uses these; each
        account brings its own key for that, under Settings.
      </p>
      <div class="button-row">
        <Button
          label="Use my provider"
          icon="pi pi-copy"
          severity="secondary"
          outlined
          title="Copy the provider, model and API key from your own Settings"
          :loading="adopting"
...
/* Not `.page-heading`: that class carries an eyebrow rule that uppercases its <p>, which
   turned this explanation into a paragraph of shouting. */
.heading-note {
  margin: 0;
  max-width: 44rem;
  text-transform: none;
